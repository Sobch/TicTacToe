...
$(function(){
	...
	function windowResize(){
		size = $(window).width()+150>$(window).height()?$(window).height()-250:$(window).width()-100;
		$(".grid").css({"width": size, "height": size});
		if (18*(size/200)>10) $("#game h1").css("font-size",(18*(size/200)));
		if (size>=600) $(".grid").css("border-spacing","3px");
			else if (size<600 && size>=320) $(".grid").css("border-spacing","2px");		
			else if (size<320) $(".grid").css("border-spacing","1px");
	}
	...
	if (!gameOn) { alert("You messed in DOM!"); return; };
		if (!$(this).hasClass("X") && !$(this).hasClass("O")) {
			...
			if (lastX) {
				lastX=false;
				$(this).addClass("O");
				grid[$(this).index()][$(this).parent().index()]=-1;
				if (isWinner(-1,X,Y)) { $("#turn").text("O wygrał!"); setTimeout(function(){ goToEndOptions(-1); }, 1000); return; }
				$("#turn").text("Kolej na X");
			} else {
				lastX=true;
				$(this).addClass("X");
				grid[$(this).index()][$(this).parent().index()]=1;
				if (isWinner(1,X,Y)) { $("#turn").text("X wygrał!"); setTimeout(function(){ goToEndOptions(1); }, 1000); return; }
	...
	function onRadioClick(){
		...
		} else { $('#toWin4').prop("disabled", false); $('#toWin5').prop("disabled", false); }
		
		if ($(this).attr('id') == "toWin4" || $(this).attr('id') == "toWin5") {
			$('#gridSize3').prop("disabled", true);
		} else { $('#gridSize3').prop("disabled", false); }
	}
